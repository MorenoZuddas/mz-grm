...
const equipmentCardBackground: 'default' | 'soft' | 'sky' | 'glass' | 'navy' = 'soft';
...
export default async function TrekkingEquipmentPage() {
  const items = await hydrateEquipmentFromCloudinary(getEquipmentBySection('trekking'));
  ...
  return (
    <PageShell background="sky">
      <section className="relative w-full h-[34vh] sm:h-[38vh] overflow-hidden trek-eq-hero-1" data-testid="trek-eq-hero-1">
        <div
          className="absolute inset-0 bg-cover bg-center scale-105 trek-eq-hero-bg-1"
          style={{
            backgroundImage:
              'url(https://res.cloudinary.com/derbnvxif/image/upload/q_auto/f_auto/v1777450410/trekking_h2lev5.jpg)',
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/55 to-black/20 trek-eq-hero-overlay-1" />

        <Link
          href="/exploration/trekking"
          className="absolute top-6 left-6 sm:left-10 inline-flex items-center gap-1.5 text-white/85 hover:text-white text-sm font-medium transition z-10 trek-eq-back-link-1"
          data-testid="trek-eq-back-link-1"
        >
          ← Trekking
        </Link>

        <div className="absolute inset-0 flex flex-col items-center justify-end px-6 pb-7 sm:px-10 sm:pb-8 trek-eq-hero-content-1" data-testid="trek-eq-hero-content-1">
          <div className="w-full max-w-2xl space-y-2 mb-5 text-center trek-eq-hero-text-1" data-testid="trek-eq-hero-text-1">
            <Text as="h1" variant="title" tone="white" align="center" className="text-4xl sm:text-5xl lg:text-6xl leading-tight trek-eq-title-1" data-testid="trek-eq-title-1">
              Attrezzatura Trekking
            </Text>
            <Text as="p" variant="body" tone="white" align="center" className="text-sm sm:text-base text-white/85 max-w-xl mx-auto trek-eq-subtitle-1" data-testid="trek-eq-subtitle-1">
              Equipaggiamento, calzature e accessori usati su sentieri, dislivelli e uscite outdoor.
            </Text>
          </div>
        </div>
      </section>
